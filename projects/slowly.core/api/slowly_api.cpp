...
slowly_package core::match_in_slowly(keywords key)
{
    switch (key) {
    case keywords::loop:
        return loop_package;
...
slowly_package core::match_in_slowly(const std::string& name)
{
...
codes core::add_tokens(const std::filesystem::path& file)
{
...
codes core::translate(std::filesystem::path sl_file)
{
    // check if file exists
